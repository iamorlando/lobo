...
impl Decimal {
    ...
    pub fn compare(&self, other: &Self) -> std::cmp::Ordering {
        ...
        let mut left = self.coefficient.to_string();
        let mut right = other.coefficient.to_string();
        let order =
            (left.len() as i32 - self.scale as i32).cmp(&(right.len() as i32 - other.scale as i32));
        if !order.is_eq() {
            return order;
        }
        let length = left.len().max(right.len());
        left.extend(std::iter::repeat_n('0', length - left.len()));
        right.extend(std::iter::repeat_n('0', length - right.len()));
        left.cmp(&right)
    }
    ...
    pub fn atoms(&self, decimals: u8) -> Result<u64, String> {
        let value = if self.scale > decimals as u32 {
            ...
            self.coefficient
                .checked_mul(
                    10u128
                        .checked_pow(decimals as u32 - self.scale)
                        .ok_or("Unsupported decimal precision")?,
                )
                .ok_or("Decimal value overflow")?
        };
        u64::try_from(value)
            .map_err(|_| "Decimal exceeds 64-bit quantity/price storage".into())
    }
}
