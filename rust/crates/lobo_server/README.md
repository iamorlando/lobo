...
Build the extension and packaged terminal with `make py` (or `make py-release`).
This builds the existing Next.js app as static assets and includes its WebAssembly
module in the Python package. Launching a server requires no Node.js process.
...
[Axum](https://docs.rs/axum/latest/axum/) handles HTTP/WebSocket transport and
Tokio owns the runtime and bounded broadcast channel. Each request is decoded
directly into its Rust command, validated at the adapter boundary, and applied
through existing storage calls with concrete publishers. No new book or level
implementation is used. CPU matching runs outside the Tokio network workers.

The mutation lock also covers sequence assignment and publication. WebSocket
receivers subscribe before snapshots; sequence numbers discard updates already
contained in a snapshot and detect gaps. Slow clients reconnect for fresh state
instead of blocking book mutation. Simulated API commands are excluded from the
main feed, and browser alternate timelines remain isolated from server state.

`make web-server-assets` builds just the WASM/static terminal package. Include
those assets when building a wheel/sdist directly with Maturin. The generated
directory is `python/lobo/_web`; source assets remain under `web/`.

Run focused checks with:

```sh
cargo test -p lobo_server --features feed-tests
poetry run pytest python/tests/lobo/test_server.py
```
